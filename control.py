...
class listner(threading.Thread):
    ...
    def run(self):
        ...
        while True:
			variables = receiveCommand([clientsocket])
			status_update()
            if self.end == True:
                break
    def join(self):
...
def removeContestant(contestantIndex):
    ...
    send(contestantIndex, clientsocket)
...
ttk.Button(mainFrame, text="Correct", command=lambda: send(1, clientsocket)).grid(column=2, row=1, sticky=N)
ttk.Button(mainFrame, text="Incorrect", command=lambda: send(2, clientsocket)).grid(column=2, row=2, sticky=N)
ttk.Button(mainFrame, text="Bank", command=lambda: send(3, clientsocket)).grid(column=3, row=2, sticky=N)
ttk.Button(mainFrame, text="Time Up", command=lambda: send(4, clientsocket)).grid(column=3, row=1, sticky=N)
